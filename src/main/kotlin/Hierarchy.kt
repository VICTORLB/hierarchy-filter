 ...
 * it passes the predicate AND all its ancestors pass as well.
 * Time: O(n). Extra state (besides output arrays): O(1).
 */
fun Hierarchy.filter(nodeIdPredicate: (Int) -> Boolean): Hierarchy {
    ...
    var blockedDepth = -1 // -1 => not inside a pruned subtree

    for (i in 0 until size) {
        val d = depth(i)
        val id = nodeId(i)

        // Still inside a pruned subtree? skip
        if (blockedDepth != -1 && d > blockedDepth) continue
        // Left the pruned subtree? clear the block
        if (blockedDepth != -1 && d <= blockedDepth) blockedDepth = -1

        val pass = nodeIdPredicate(id)
        if (pass) {
            outIds[outSize] = id
            outDepths[outSize] = d
            outSize++
        } else {
            blockedDepth = d // prune this node's subtree
        }
    }

    return ArrayBasedHierarchy(outIds.copyOf(outSize), outDepths.copyOf(outSize))
}
...
class ArrayBasedHierarchy(
    ...
    init {
        require(myNodeIds.size == myDepths.size) {
            "nodeIds and depths must have same length: ${myNodeIds.size} vs ${myDepths.size}"
        }
    }
